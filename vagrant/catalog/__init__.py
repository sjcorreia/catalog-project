...
engine = create_engine('postgresql+psycopg2://catalog:udacity@localhost/catalog_db')
...
def addNewCatalogItem():
    if ('user_token' in login_session and
            verify_password(login_session['username'], 'password')):
        categories = session.query(Category).all()
        if request.method == 'POST':
            newItem = CatalogItem()
